...
This is the story of building **Sehat Sathee** (Health Companion) in 36 hours—and the technical challenges that kept us awake. 

*Editor's note (2026-09-03): kept as originally drafted, not silently corrected. Two things worth an
owner's second look before treating this as current — the rural-population and internet-access
figures in the paragraph above are unsourced and may be out of date; and the CoWIN integration
described in "Challenge #4" below queries `cdn-api.co-vin.in`, a public endpoint India restricted
after its mass COVID-vaccination drive wound down, so it may no longer have worked as written by the
time of the hackathon. Separately, the source draft this post was written from cut off mid-Dockerfile
with no closing section — the fenced code block below has been closed so the page renders correctly,
but nothing past that point was invented to fill the gap.*
...
- "टीका लगवाना है" (Hindi:  "Want to get vaccinated")
- "कब खुली है centre?" (Hinglish mix)
...
VaccinationAppointment: 
  ...
  - "I need vaccine"
  - "टीका कब मिलेगा"  # Hindi
  - "ವ್ಯಾಕ್ಸಿನೇಶನ್ ಎಲ್ಲಿದೆ"  # Kannada
  - "टीका बुक करा"  # Marathi

DiseaseOutbreakAlert: 
  - "Any disease in my area?"
  - "क्या मेरे क्षेत्र में कोई बीमारी है"
  - "ਮੇਰੇ ਇਲਾਕੇ ਵਿੱਚ ਕੋਈ ਬਿਮਾਰੀ"  # Punjabi
```
...
3. Backend processes message → Dialogflow NLP
...
```javascript
// ❌ Bad: Send image with vaccination center locations
twiml.message().media('https://maps.googleapis.com/.. .')

// ✅ Good: Send text directions
twiml.message(`
📍 Nearest Center:
...
*(The source draft ends here — see the editor's note above.)*