...
npm run agent-data     # rebuild public/agent-data.json without starting dev
...
**`agent-data.json` is built the same way, for a different consumer.** `scripts/build-agent-data.mjs`
runs in the same `predev`/`prebuild` hook and writes `public/agent-data.json` (gitignored, like
`llms.txt`) — a structured `{profile, projects[]}` JSON export of the same `person`/`story` copy and
project list `llms.txt` renders as prose. It exists for the future MCP server (issue #62, effort 045,
not yet built) to fetch and back its `getProfile`/`searchProjects` tools; `llms.txt` stays the
prose/crawler-facing sibling. Pure renderer: `scripts/lib/agent-data.mjs`
(`renderAgentData(site, projects) -> {profile, projects}`), unit-tested by `npm run test:unit`; the
project shape matches `src/lib/agentProjects.ts`'s `AgentProject` (slug, title, summary, year, url,
code?) so there is one project shape shared across `llms.txt`, WebMCP, and this export, not three.
...
| `public/agent-data.json` | Generated by `scripts/build-agent-data.mjs`; gitignored, regenerated every build |
