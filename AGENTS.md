...
npm run resume:build   # regenerate resume/resume.tex from site.ts, compile to public/resume.pdf if
                        # pdflatex/latexmk is on PATH (prints a clear message and skips the compile if not)
npm run resume:verify  # extract public/resume.pdf's text and assert it contains name/roles/GPA from site.ts
...
factuality eval's optional judge tier, which is skipped when no API key is set), `pdf-parse` (extracts
`public/resume.pdf`'s text for `npm run resume:verify`; the LaTeX toolchain itself — `pdflatex`/`latexmk`
locally, `xu-cheng/latex-action` in CI — is not an npm dependency). Fonts: Instrument Serif (italic
...
- `.github/workflows/evals.yml` — on PR to `main`, **only** when the diff touches `content/projects/**`,
  ...
  factuality failure.
- `.github/workflows/resume-ci.yml` — on PR to `main`, only when the diff touches `resume/**`,
  `scripts/lib/resume-tex.mjs`, `scripts/build-resume.mjs`, `scripts/verify-resume-pdf.mjs` or `site.ts`:
  regenerates `resume/resume.tex` from `site.ts` and fails if that leaves it out of sync with the
  committed file, compiles it via `xu-cheng/latex-action`, then verifies the compiled PDF's extracted
  text contains the current name/roles/companies/GPA. ADR 0016.
- `.github/workflows/resume-auto-refresh.yml` — on push to `main` touching `src/content/site.ts` (or
  manual dispatch): regenerates and compiles the résumé, and — only if the compiled PDF is
  byte-different from the one committed — opens a review-only PR with the new `resume/resume.tex`,
  `public/resume.pdf`, and an auto-drafted AI-DLC effort-record skeleton. Never merges its own PR. ADR 0016.
...
files fall back to a kebab-case of the filename).

**The résumé is built too — a second instance of the same convention, not hand-copied since effort 028.**
`resume/resume.tex` (Jake's Resume template, MIT-licensed) is the source; its Header, Education and
Experience sections are marked `% AUTO-GENERATED:<NAME>-START/END` and generated by
`node scripts/build-resume.mjs` from `person`/`experience`/`story.education` in `src/content/site.ts`
via the pure renderer `scripts/lib/resume-tex.mjs` (unit-tested, `npm run test:unit`, no LaTeX toolchain
needed). Projects, Technical Skills, and Key Achievements stay hand-maintained LaTeX in the same file —
`site.ts` has no typed field for that level of résumé-specific detail (exact metrics, named tools); edit
`resume/resume.tex` directly for those. `npm run resume:build` regenerates the `.tex` and compiles it to
`public/resume.pdf` with `pdflatex`/`latexmk` if found on `PATH` (prints a clear message, doesn't hard-fail,
if neither is present); `public/resume.pdf` is compiled by CI (`xu-cheng/latex-action`) and **committed**
like `public/brain/**` — `deploy.yml`'s runner has no TeX toolchain, so this one drop-zone-style path is
not gitignored the way `public/img/**` is. The old root `resume.pdf` drop-zone is retired: edit
`src/content/site.ts` for career facts, or `resume/resume.tex` directly for the hand-maintained sections.
See ADR 0016 and `.github/workflows/resume-auto-refresh.yml`, which opens a review-only PR when a
career-fact change in `site.ts` produces a different compiled PDF.
...
3. `npm run test:smoke` — passes against that `out/`. If you touched layout, images, motion, scripts or
   ...
   *Evals / factuality* (ADR 0013).
   Touched `resume/`, `scripts/lib/resume-tex.mjs`, or `scripts/build-resume.mjs`? `npm run resume:build`
   locally if a LaTeX toolchain is available; CI compiles and verifies via *Resume* (ADR 0016).
