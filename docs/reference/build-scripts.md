...
Files in `scripts/`. Most run under `node`, one under Python.
...
| `build-resume.mjs` | `src/content/site.ts`, `resume/resume.tex` | `resume/resume.tex` (in place, marker sections only), `public/resume.pdf` (if a LaTeX toolchain is on `PATH`) | `npm run resume:build` |
| `lib/resume-tex.mjs` | nothing (pure) — `person`/`education`/`experience` data in, LaTeX fragments out | nothing (pure) | imported by `build-resume.mjs`; unit-tested by `resume-tex.test.mjs` |
| `verify-resume-pdf.mjs` | a compiled `resume.pdf`, `src/content/site.ts` | nothing (exit code) | `npm run resume:verify`; `.github/workflows/resume-ci.yml`, `resume-auto-refresh.yml` |
| `resume-auto-effort.mjs` | `aidlc-docs/registry.md`, `aidlc-docs/audit.md` | a new `aidlc-docs/efforts/NNN-resume-auto-refresh-*/` folder, an appended registry row and audit row | `.github/workflows/resume-auto-refresh.yml`, only when it is about to open a PR |
...
- **Behaviour:** lists files changed between `baseRef` and `HEAD`. If any match a *substantive* pattern (`src/`, `scripts/`, `content/`, `resume/`, `package.json`, `package-lock.json`, `next.config.*`, `tsconfig.json`, `.github/workflows/`) and are not *exempt* (`public/`, `out/`, `gallery/`, `project_images/`, `me.jpg`), then at least one changed path must start with `aidlc-docs/` — otherwise exit 1 with the list of offending files and the five-step lifecycle.
...
To change any of these, change the source: the drop-zone, the script, or `src/content/site.ts`.

`public/resume.pdf` is generated by `build-resume.mjs` (locally) or CI (`resume-ci.yml`,
`resume-auto-refresh.yml`) from `resume/resume.tex`, but is **committed** rather than gitignored,
because `deploy.yml`'s runner carries no LaTeX toolchain to regenerate it at deploy time — the same
reason `public/brain/**` is committed. The old convention (hand-copy the root `resume.pdf` into
`public/`) is retired as of effort 028; see ADR 0016.
