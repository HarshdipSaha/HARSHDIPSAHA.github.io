...
Effort 036 (this one) was assigned and recorded in its own worktree while efforts 034, 035 and
037-040 were each running concurrently in sibling worktrees against the same `origin/main`, per the
brief that dispatched all of them together. This registry copy therefore has a numbering gap at
034-035 rather than those efforts' real rows; each of those efforts records itself the same way in
its own worktree, and whoever integrates the branches reconciles this file into one ordered list —
consistent with how 026-028's interleaving above was resolved.

Next effort number: 037
