...
| `/writing` | `src/app/writing/page.tsx`; `writing` in `site.ts`; 3 MDX files in `content/writing/` | Live — list of writing posts, newest first |
| `/writing/[slug]` | `src/app/writing/[slug]/page.tsx`; slug = lowercased MDX filename | Live — one page per post, MDX body via `next-mdx-remote/rsc` (ADR 0016) |
...
| `/sitemap.xml`, `/robots.txt` | `src/app/sitemap.ts`, `src/app/robots.ts` (`force-static`) | Live — sitemap = `/` + `nav` + every project slug + every writing-post slug |
| `/llms.txt`, `/llms-full.txt` | Generated by `scripts/build-llms-txt.mjs` on `prebuild` into `public/` | Live — agent-facing, not routes: no `nav` entry, not in the sitemap (ADR 0014). Includes a Writing section since effort 038 |
