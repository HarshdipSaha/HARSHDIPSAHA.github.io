...
| `/agent-data.json` | Generated by `scripts/build-agent-data.mjs` on `prebuild` into `public/` | Live static file, not a route — structured sibling to `llms.txt`, for the future MCP server (issue #62) to fetch |
...
site.ts +        ->  scripts/build-agent-data.mjs    ->  public/agent-data.json    ->  the future MCP server
content/projects/    (pure renderer in                   (gitignored)                  (issue #62, effort 045,
*.mdx                 scripts/lib/agent-data.mjs)                                       not yet built)
```
...
| `agent-data.json` | A structured `{profile, projects[]}` JSON export, generated at build time alongside `llms.txt`, for the future MCP server (issue #62) to fetch — the queryable sibling to `llms.txt`'s prose. |
