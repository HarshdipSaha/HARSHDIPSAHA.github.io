...
| Unverifiable | A claim in a case study with no fetchable source — no `link` in frontmatter (BrainwavesFinland and SAAKSHI, whose repositories are private), or a source that returns 404. Reported by name, never silently skipped; does not fail the run. |
